...
function ChucklePostAI(config) {
    ...
    function bindButtonEvents(aiContainer) {
      ...
      generateIntroductionButton.addEventListener("click", () => {
        disableButtons(aiContainer); // 禁用按钮
        displaySummary(
          "我是文章辅助AI，使用的OpenAI的gpt-3.5-turbo, embedding, gpt-4o-mini, gpt-4模型。点击下方的按钮，让我生成本文简介。",
          aiContainer
        );
      });
    ...
    async function generateSummary(aiContainer) {
      ...
      const apiKey = "sk-X0SGzTCLenEtBroX1Nm5nItG1iYP32zAtUKyyT2im62MYWFD"; // 请替换为你的实际 API KEY
      const requestBody = {
